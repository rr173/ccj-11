...
## 电子回执离线核验设备与增量同步（offline verification devices）

主管可登记专用离线核验设备，选择该设备允许核验的回执范围（全部或指定回执编号）、授权有效期与撤销宽限期，并生成**只能下载一次**的离线授权包；设备在断网时凭包内信息核验回执，重新联网后按游标分批上传日志并拉取增量。

### 授权包（自包含、服务器签名、最小脱敏）

- 每台设备每次授权使用一把独立 Ed25519 密钥对：私钥以 `0600` PEM 持久化在服务器数据目录（或经 `OFFLINE_SIGN_KEY_PEM` 注入），**永不外泄**；授权包只内嵌公钥（SPKI）。轮换授权时生成全新密钥，旧私钥文件立即销毁。
- 授权包是 `{payload, signature}` 自描述 JSON：`payload` 经规范序列化（键排序、UTF-8）后由设备授权私钥签名，设备用包内公钥在完全离线状态下验签。**任何字段被篡改都会验签失败**（`PACKAGE_TAMPERED`）。
- payload 含：设备标识/名称、授权版本、范围、签发/到期时间、撤销宽限、基线游标、公钥，以及逐条脱敏回执记录（脱敏姓名、脱敏手机号、事项类型、完成时间、签发/撤销状态、核验码摘要、逐条摘要）。
- **白名单脱敏**：授权包只允许出现核验必需字段；`assertNoForbiddenFields` 在发包前递归检查，证件号、完整地址、完整手机号、办理备注（`matter.description`）、各步原始申报数据一律不进入授权包（测试固化为不得出现 `idNumber/phone/address/detail/description/steps/note/snapshot`）。
- 下载凭证为随机码（仅存 SHA-256 哈希），默认 15 分钟有效、**只能成功兑换一次**：兑换成功即把凭证置 `used`、授权包置 `downloaded`；再次下载返回 `PACKAGE_ALREADY_DOWNLOADED`。轮换/停用后未下载凭证立即 `AUTHORIZATION_ROTATED`。
- 设备同步令牌为随机串（仅存哈希），轮换时旧令牌置 `rotated`；停用或轮换后旧令牌的任何同步都返回 `AUTHORIZATION_ROTATED`（HTTP 401）。

### 离线核验与明确拒绝

设备端（`src/offlineDevice.js`，浏览器与 Node 测试共用，仅依赖 Web Crypto）在以下情形**明确拒绝**：

- 授权包验签失败 / 与设备不匹配（`PACKAGE_TAMPERED` / `PACKAGE_DEVICE_MISMATCH`）；
- 设备已停用、已过期（`DEVICE_DISABLED` / `DEVICE_EXPIRED`）；
- 超过本机“必须再次同步”期限仍未联网（拒绝原因 `sync_overdue`，拒绝同样记入摘要链）：期限取服务器下发的 `mustSyncBefore` 与“上次成功同步（或授权包签发）+ 撤销宽限期”的较早者——离线设备无法察觉撤销，宽限期只能自上次同步起算，超期即本机立即拒绝，而不是等日志上传后才被服务器判失效；
- 回执不在授权范围（`out_of_scope`）、不存在（`not_found`）、已撤销（`revoked`）、核验码摘要不符（`code_mismatch`）。

通过的核验只展示脱敏字段；每次核验（无论通过或拒绝）都在本机按**递增序号**记录日志，每条摘要为 `SHA256(规范条目含前一条摘要)`，形成逐条串联的摘要链。本机状态可序列化持久化（刷新/重启后恢复游标、链头、待传日志）。

### 增量同步（游标、幂等批次、链式校验）

重新联网后设备以 `Bearer` 设备令牌调用 `/api/offline/sync`，请求含设备标识、授权版本、客户端游标、批次号与日志条目；服务器在单个 `BEGIN IMMEDIATE` 事务内完成上传校验与增量下发：

- **身份/版本**：令牌、`deviceId`、`keyVersion` 任一不符即拒（`DEVICE_TOKEN_INVALID` / `IDENTITY_MISMATCH` / `KEY_VERSION_MISMATCH`）。
- **游标**：游标在 `offline_device_state` 持久化；客户端游标领先服务器 = 伪造/倒退 → `CURSOR_REGRESSED`。空批次（只拉取）允许落后补拉；带日志的新批次必须先追平服务器游标（基于同一撤销视图记日志）。每次成功同步把下发到的新游标落库，重启不丢。
- **幂等批次**：同一 `(deviceId, batchId)` 的重放必须内容指纹（规范序列化）完全一致；一致则幂等返回、不重复落库、不覆盖已收日志；不一致 → `BATCH_DUPLICATE_CONFLICT`。
- **日志链校验**：序号必须紧接服务器已收序号（缺口 → `LOG_GAP`）；每条 `prevDigest` 必须承接服务器链头（分叉 → `LOG_FORK`）；服务器逐条重算摘要（不符 → `LOG_DIGEST_INVALID`）；同序号内容不同 → `LOG_DUPLICATE_CONFLICT`；未来时间 → `ENTRY_TIME_INVALID`。整批任一不符都回滚，**服务器已接收日志永不被覆盖**（`UNIQUE(device_id,key_version,seq)` 兜底）。
- **范围复核**：结果为“通过”的日志，其回执必须仍在该设备授权范围内。
- 同步响应按设备范围下发 `offline_feed` 增量（新签发回执与撤销，仍为脱敏记录）与新游标；设备应用增量后，被撤销回执在后续离线核验中**立即拒绝**。

### 撤销宽限期

回执在设备离线期间被撤销时，主管可为设备设置短暂宽限期（默认 5 分钟，可配置 1–60 分钟）：设备在宽限期内基于旧视图做出的通过核验，重新联网时服务器仍接受并立即下发撤销；**超过撤销时刻 + 宽限才做出的“通过”日志整批拒绝**（`REVOKED_BEYOND_GRACE`）。由于离线设备无法察觉撤销，本机宽限期一律自上次成功同步（或授权包签发）起算：超过“上次成功同步 + 宽限期”与服务器下发 `mustSyncBefore` 的较早者仍未同步，设备**本机立即拒绝一切核验**（拒绝原因 `sync_overdue`，记入摘要链日志），而不是等日志上传服务器后才发现结果失效；一旦撤销随增量送达，该回执在本机视图即为已撤销，后续离线核验**继续立即拒绝**（`revoked`）；完成同步后期限自新的同步时刻顺延，未撤销回执恢复可核验。

### 设备丢失：停用与授权轮换

- 停用：设备置 `disabled`、当前令牌作废、未下载/已下载授权包标记 `rotated`、私钥文件销毁；旧令牌与未上传日志不再被接受。
- 轮换（疑似泄露/设备找回前）：`key_version` 递增，换新令牌与新签名密钥，旧包/旧令牌立即失效，日志链按新授权版本重新从序号 1 开始（旧版本日志保留审计）。

### 三种角色视图与审计

- 主管页 `/offline-admin`：登记设备、选择范围/有效期/宽限期、查看设备状态、最后同步游标、已收日志序号、待同步增量数、最近分叉/缺口原因；生成一次性授权包、轮换、停用、查看设备日志与审计。
- 审计员页 `/auditor`：新增“离线核验设备事件审计”区块，可查看每次发包（issued/downloaded）、离线核验（accepted/rejected）、同步（含幂等重传）、各类拒绝、轮换与停用事件；审计表 `offline_audit` 只追加，数据库触发器禁止 UPDATE/DELETE。
- 办理人：`/api/receipts` 与 `/api/receipts/offline-verifications` 只返回本人回执是否曾被离线核验（回执编号、结果、拒绝原因、时间、设备 id/名称），**看不到设备令牌、下载凭证、授权包或他人回执**。

设备端页面 `/offline-device`：录入设备令牌与一次性凭证下载并验签授权包；断网模式核验；联网模式按游标分批同步、幂等重试上一批次；本机状态保存在浏览器存储，刷新/重新登录后续传一致。
...
| `OFFLINE_SIGN_KEY_PEM` | 空 | 离线授权包 Ed25519 签名私钥（PEM）。设置后所有设备共用该注入密钥且不写私钥文件；留空则为每台设备每次授权在数据目录生成独立 0600 私钥文件，轮换即销毁旧文件 |
| `OFFLINE_SIGN_KEY_PATH` | `data/` 目录 | 离线签名密钥文件所在目录（每设备每授权版本一个 `offline-dev-<deviceId>-k<version>.pem`） |
| `OFFLINE_PACKAGE_CREDENTIAL_TTL_MS` | `900000` | 授权包一次性下载凭证有效期（15 分钟） |
| `OFFLINE_SYNC_MAX_LOGS` | `500` | 单次同步最多接受的核验日志条数 |
| `OFFLINE_SYNC_MAX_DELTA` | `500` | 单次同步最多下发的回执增量条数（超出以 `hasMore` 分批） |
| `OFFLINE_GRACE_MIN_MS` / `OFFLINE_GRACE_MAX_MS` | `60000` / `3600000` | 撤销宽限期允许范围（1 分钟–1 小时） |
| `OFFLINE_GRACE_DEFAULT_MS` | `300000` | 默认撤销宽限期（5 分钟） |
| `OFFLINE_TTL_MIN_MS` / `OFFLINE_TTL_MAX_MS` | `600000` / `31536000000` | 设备授权有效期允许范围（10 分钟–1 年） |
| `OFFLINE_TTL_DEFAULT_MS` | `2592000000` | 默认授权有效期（30 天） |
