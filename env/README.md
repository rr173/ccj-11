...
- 超过撤销同步期限仍未联网（`REVOKED_BEYOND_GRACE`，本机自我停用核验）；
...
回执在设备离线期间被撤销时，主管可为设备设置短暂宽限期（默认 5 分钟，可配置 1–60 分钟）：设备在宽限期内基于旧视图做出的通过核验，重新联网时服务器仍接受并立即下发撤销；**超过撤销时刻 + 宽限才做出的“通过”日志整批拒绝**（`REVOKED_BEYOND_GRACE`）。同步响应在仍有未送达撤销时给出 `mustSyncBefore`，设备超过该期限未同步会自我停用核验；一旦撤销随增量送达，截止时间清零且本机立即拒绝该回执。
